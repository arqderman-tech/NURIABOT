...
<title>Nuria Price Tracker</title>
...
:root{{--bg:#0a0c12;--surface:#13161f;--surface2:#1a1d27;--border:#252836;--accent:#ec4899;--text:#e2e8f0;--muted:#64748b;}}
...
.hero{{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:0.75rem;margin-bottom:1.5rem;}}
...
<header>
  <div>
    <h1>🎂 NURIA PRICE TRACKER</h1>
    <div class="tagline">Pastelería y Confitería Nuria — Rosario</div>
  </div>
...
<footer>Datos de nuria.com.ar · NURIABOT · Actualización diaria 12:00 ART via GitHub Actions</footer>

<script>
const G = {graficos_js};
const RANK = {{dia:{rank_dia_js},d7:{rank_7d_js},mes:{rank_mes_js}}};
const CAT_COLORS = {{"Tortas Individuales":"#ec4899","Tortas y Postres":"#f43f5e","Pasteleria y Panaderia":"#f59e0b","Salados":"#10b981","Otros":"#6366f1"}};
...
function renderMain(p){{
  ...
  chartMain=new Chart(document.getElementById('chartMain').getContext('2d'),{{
    type:'line',
    data:{{labels:d.map(x=>x.fecha),datasets:[{{label:'Total',data:d.map(x=>x.pct),borderColor:'#ec4899',backgroundColor:'rgba(236,72,153,0.06)',borderWidth:2,pointRadius:d.length>60?0:2,tension:0.3,fill:true}}]}},
...
renderMain('7d'); renderCats('7d'); mostrarRanking('dia');
    ...
    with open(DIR_DOCS/"index.html","w",encoding="utf-8") as f:
        f.write(html)
    print("Web NURIABOT generada: docs/index.html")
